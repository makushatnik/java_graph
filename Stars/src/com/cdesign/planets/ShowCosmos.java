...
class PlanetsCanvas extends JPanel {
	...
	private Planet[] planets = { new Planet("Venera", 1, 5.0, 50.0, 3, 0, Color.GREEN),
			new Planet("Mercury", 2, 3.0, 75.0, 4, 0, Color.ORANGE), new Planet("Earth", 3, 6.0, 100.0, 2, 1, Color.BLUE),
	...
	private Color[] colors = { Color.WHITE, Color.LIGHT_GRAY, Color.GRAY, Color.DARK_GRAY, Color.YELLOW, Color.ORANGE,
			Color.RED, Color.PINK, Color.MAGENTA, Color.GREEN, Color.BLUE, Color.CYAN };
	...
	protected void drawFrame(Graphics g) {
		...
		// new
		/*
		 * if(nStar > STARS_MAX) { for(int i=STARS_MAX; i<nStar; i++) { stars[i] =
		 * null; stars[i].destroy(); } nStar = STARS_MAX; } else
		 */
		...
		for (int i = 0; i < planets.length; i++) {
			...
			int newX = (int) (centerX + Math.cos(p.getAngle() * Math.PI / 180) * p.getRadiusOrbit());
			int newY = (int) (centerY
					- Math.sin(p.getAngle() * Math.PI / 180 * (planets[i].getDirection() == 0 ? 1 : -1)) * p.getRadiusOrbit());
			g.fillOval((int) (newX - p.getRadius()), (int) (newY - p.getRadius()), (int) p.getRadius() * 2,
					(int) p.getRadius() * 2);
